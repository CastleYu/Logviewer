...
export const AiSettings: React.FC<AiSettingsProps> = ({ theme, open, onClose, onSaved, initial, catalog, repository }) => {
  ...
  const patch = (part: Partial<SettingsConfig>) => { setDraft((value) => ({ ...value, ...part })); setError(null); };
  ...
  return <FloatingPanel theme={theme} title="AI 设置" open={open} onClose={onClose} width={540} height={600}>
    <form onSubmit={(event) => { event.preventDefault(); void save(); }} className="h-full space-y-3 overflow-auto p-4">
      ...
      <label className="block"><span className="mb-1 block text-xs font-semibold">服务地址</span><input aria-label="服务地址" value={draft.endpoint} onChange={(event) => patch({ endpoint: event.target.value })} className={field} /></label>
      <label className="block"><span className="mb-1 block text-xs font-semibold">本地启动程序</span><input aria-label="本地启动程序" value={draft.executable} onChange={(event) => patch({ executable: event.target.value })} className={field} /></label>
      <label className="flex items-center gap-2 text-xs"><input type="checkbox" checked={draft.launch} onChange={(event) => patch({ launch: event.target.checked })} />需要时启动本地服务</label>
