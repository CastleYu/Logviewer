      ...
      <div style={{ minWidth, height: Math.max(totalHeight + 30, height) }} className="relative">
        ...
        <div style={{ transform: `translateY(${start * itemHeight}px)` }} className="absolute left-0 right-0 top-[30px]">
          {visibleLogs.map((entry, offset) => {
            ...
            return <div key={entry.id} style={{ height: itemHeight }} onClick={(event) => selectRow(entry, index, event)} onContextMenu={(event) => { event.preventDefault(); event.stopPropagation(); if (!selectedIds.has(entry.id)) onSelectionChange(new Set([entry.id])); setContextTarget({ field: sourceField(event), x: Math.min(event.clientX, window.innerWidth - 240), y: Math.min(event.clientY, window.innerHeight - 260), log: entry }); }} className={`flex items-stretch border-b text-[11px] font-mono cursor-pointer ${levelClass(entry.fields?.level || '', isLight)} ${selected ? isLight ? 'outline outline-2 -outline-offset-2 outline-indigo-500 bg-slate-200/80' : 'outline outline-2 -outline-offset-2 outline-indigo-500 bg-slate-800/80' : ''} ${activeSearch ? 'ring-2 ring-inset ring-indigo-500' : ''}`}><div className="w-14 shrink-0 px-2 flex items-center justify-center text-slate-400 border-r border-inherit"><StackCell log={entry} onOpen={onOpenStack} /></div>{fields.map((field) => { const text = entry.stack && field.role === FieldRole.Message ? `${stackSummary(entry.stack)} · ${frameCount(entry.stack)} 帧` : entry.success ? cellText(entry.fields?.[field.id]) : field.role === FieldRole.Message ? entry.rawText : '-'; const cellAction = copyActionsAt(actions, CopyPlacement.Cell, field.id)[0]; return <div key={field.id} data-source-field={field.id} style={{ width: field.display?.width || 140, flexGrow: field.display?.grow ? 1 : 0 }} className={`group/cell shrink-0 min-w-0 px-2 flex items-center gap-1 border-r border-inherit ${alignClass(field)}`} title={text}><span className="min-w-0 flex-1 truncate">{entry.stack && field.role === FieldRole.Message ? <button type="button" onClick={(event) => { event.stopPropagation(); onOpenStack(entry); }} className="min-h-6 max-w-full truncate text-left underline decoration-dotted underline-offset-4 focus-visible:outline focus-visible:outline-2 focus-visible:outline-indigo-500" aria-label={`查看堆栈：${text}`}>{text}</button> : <HighlightedText text={text} highlight={filter.highlightKeyword} matchCase={filter.highlightMatchCase} isRegex={filter.highlightIsRegex} searchHighlight={filter.searchKeyword} searchMatchCase={filter.matchCase} searchIsRegex={filter.isRegex} pinnedHighlights={filter.pinnedHighlights} theme={theme} />}</span>{cellAction ? <button type="button" onClick={(event) => { event.stopPropagation(); runCopy(cellAction, [entry]); }} aria-label={cellAction.label} title={cellAction.label} className="opacity-0 group-hover/cell:opacity-100 focus-visible:opacity-100 shrink-0 p-0.5 rounded text-slate-400 hover:text-indigo-500 cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500"><Copy className="w-3 h-3" /></button> : null}</div>; })}</div>;
          })}
        </div>
      </div>
