...
app.get('/permission', (_req, res) => res.json([...permissions].map(([id, sessionID]) => ({ id, sessionID, permission: 'read', patterns: ['service.py'] }))));
app.get('/question', (_req, res) => res.json([...questions].map(([id, sessionID]) => ({ id, sessionID, questions: [{ header: '复现环境', question: '选择复现环境', options: [{ label: '测试', description: '测试环境' }, { label: '生产', description: '生产环境' }], multiple: false }, { header: '补充信息', question: '需要补充哪些证据？', multiple: true, options: [{ label: '调用链' }, { label: '配置' }] }] }))));
app.post('/permission/:id/reply', (req, res) => { const id = permissions.get(req.params.id); permissions.delete(req.params.id); if (id) finishers.get(id)?.(); res.json(true); });
app.post('/question/:id/reply', (req, res) => { const id = questions.get(req.params.id); questions.delete(req.params.id); if (id) finishers.get(id)?.(); res.json(true); });
app.post('/question/:id/reject', (req, res) => { const id = questions.get(req.params.id); questions.delete(req.params.id); if (id) finishers.get(id)?.(); res.json(true); });
...
app.post('/session/:id/prompt_async', (req, res) => {
  ...
  const finish = () => {
    ...
    if ([...permissions.values(), ...questions.values()].includes(req.params.id)) return;
    ...
    session.busy = false;
    finishers.delete(req.params.id);
  };
  finishers.set(req.params.id, finish);
  if (JSON.stringify(req.body.parts).includes('权限追问验收')) {
    permissions.set(crypto.randomUUID(), req.params.id);
    questions.set(crypto.randomUUID(), req.params.id);
  }
